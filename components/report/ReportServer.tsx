...
export async function ReportServer({ id, locale, report, storeId }: { id: string; locale: "ja" | "en"; report: Report; storeId?: string }) {
  const r = report;
  if (!r) return null;

  // Get store config if storeId is provided
  ...
  return (
    <div style={{ backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      ...
      <main style={{ maxWidth: '72rem', margin: '0 auto', padding: '2rem 1rem', paddingBottom: '6rem' }} className="py-8 pb-24">
        ...
        {/* Products Section - Store-specific or Generic */}
        <div id="products" className="mt-4">
          {storeConfig ? (
            <ReportSection title={locale === 'ja' ? `${storeConfig.name} のおすすめ商品` : `${storeConfig.name} Recommendations`}>
              <div className="accent-bar mb-3"></div>
              <StoreProductsClient
                storeId={storeId!}
